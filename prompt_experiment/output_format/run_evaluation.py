...
from config.paths import PROMPT_EXPERIMENT_EVALUATIONS_DIR, PROMPT_EXPERIMENT_FINAL_FSMS_DIR
...
STATE_OUTPUT_FILE: Final[Path] = PROMPT_EXPERIMENT_EVALUATIONS_DIR / "states_match_results.csv"

# complete transition evaluation csv file
EXACT_TRANSITION_OUTPUT_FILE: Final[Path] = PROMPT_EXPERIMENT_EVALUATIONS_DIR / "transitions_match_results_whole.csv"

# partial transition evaluation csv file
PARTIAL_TRANSITION_OUTPUT_FILE: Final[Path] = PROMPT_EXPERIMENT_EVALUATIONS_DIR / "transitions_match_results_partial.csv"


# evaluate all final FSM files
def run_evaluation() -> tuple[Path, Path, Path]:
    # load all final FSM files.
    final_fsm_files: list[Path] = sorted(PROMPT_EXPERIMENT_FINAL_FSMS_DIR.glob(f"*{FINAL_FSM_SUFFIX}.json"))

    # initialise the csv rows
