...
from config.paths import REFERENCED_CONTEXT_SPLITTING_COMBINATION_RESPONSES_DIR, REFERENCED_CONTEXT_SPLITTING_FINAL_FSMS_DIR
...
def _get_final_fsm_file_path(response_file_path: Path) -> Path:
    ...
    final_fsm_file_path: Path = REFERENCED_CONTEXT_SPLITTING_FINAL_FSMS_DIR / final_fsm_file_name

    return final_fsm_file_path


# extract and save all referenced context final FSMs.
def extract_final_fsms() -> list[Path]:
    combination_response_files: list[Path] = sorted(REFERENCED_CONTEXT_SPLITTING_COMBINATION_RESPONSES_DIR.glob("*_combination_response.json"))
