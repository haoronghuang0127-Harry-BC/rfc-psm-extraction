...
from config.paths import REFERENCED_CONTEXT_SPLITTING_PROMPTS_DIR, REFERENCED_CONTEXT_SPLITTING_SEGMENTS_DIR
...
# build extraction Prompts from the referenced context segments.
def _build_extraction_prompts(protocol_names: list[str], output_control: OutputControl) -> dict[str, list[str]]:
    ...
    for protocol in protocol_names:
        referenced_context_segments_file: Path = REFERENCED_CONTEXT_SPLITTING_SEGMENTS_DIR / f"{protocol}_referenced_context_segments.json"
        referenced_context_segments: list[RfcSegment] = load_rfc_segments(file_path=referenced_context_segments_file)
...
# save all referenced context extraction Prompts.
def _save_extraction_prompts(extraction_prompts: dict[str, list[str]]) -> Path:
    output_file: Path = REFERENCED_CONTEXT_SPLITTING_PROMPTS_DIR / "ollama_json_schema_output_extraction_prompts.json"
