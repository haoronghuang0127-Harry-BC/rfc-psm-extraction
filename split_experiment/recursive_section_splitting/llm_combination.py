...
from config.paths import RECURSIVE_SECTION_SPLITTING_COMBINATION_RESPONSES_DIR, RECURSIVE_SECTION_SPLITTING_PROMPTS_DIR
...
def _load_combination_prompts() -> dict[str, str]:
    prompts_file: Path = RECURSIVE_SECTION_SPLITTING_PROMPTS_DIR / "combination_prompts.json"
...
def _run_combination_psm(prompt_name: str, prompt: str, connection: OllamaConnection, model_config: ModelConfig, profile_name: ProfileName, model_profile: ModelProfile, output_control: OutputControl) -> Path:
    ...
    output_file: Path = RECURSIVE_SECTION_SPLITTING_COMBINATION_RESPONSES_DIR / f"{prompt_name}_combination_response.json"
...
# run recursive section combination for the selected protocols and models.
