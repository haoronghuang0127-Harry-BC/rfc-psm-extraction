...
from config.paths import RECURSIVE_SECTION_SPLITTING_PROMPTS_DIR, RECURSIVE_SECTION_SPLITTING_SEGMENTS_DIR
...
# build extraction Prompts from the recursive section segments.
def _build_extraction_prompts(protocol_names: list[str], output_control: OutputControl) -> dict[str, list[str]]:
    ...
    for protocol in protocol_names:
        recursive_segments_file: Path = RECURSIVE_SECTION_SPLITTING_SEGMENTS_DIR / f"{protocol}_recursive_section_segments.json"
...
# save all recursive section extraction Prompts.
def _save_extraction_prompts(extraction_prompts: dict[str, list[str]]) -> Path:
    output_file: Path = RECURSIVE_SECTION_SPLITTING_PROMPTS_DIR / "ollama_json_schema_output_extraction_prompts.json"
