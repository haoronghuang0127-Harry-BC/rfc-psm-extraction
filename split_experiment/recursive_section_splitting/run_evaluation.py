...
from config.paths import RECURSIVE_SECTION_SPLITTING_EVALUATIONS_DIR, RECURSIVE_SECTION_SPLITTING_FINAL_FSMS_DIR
...
STATE_OUTPUT_FILE: Final[Path] = RECURSIVE_SECTION_SPLITTING_EVALUATIONS_DIR / "states_match_results.csv"

# complete transition evaluation csv file.
EXACT_TRANSITION_OUTPUT_FILE: Final[Path] = RECURSIVE_SECTION_SPLITTING_EVALUATIONS_DIR / "transitions_match_results_whole.csv"

# partial transition evaluation csv file.
PARTIAL_TRANSITION_OUTPUT_FILE: Final[Path] = RECURSIVE_SECTION_SPLITTING_EVALUATIONS_DIR / "transitions_match_results_partial.csv"


# evaluate all recursive section final FSM files.
def run_evaluation() -> tuple[Path, Path, Path]:
    final_fsm_files: list[Path] = sorted(RECURSIVE_SECTION_SPLITTING_FINAL_FSMS_DIR.glob(f"*{FINAL_FSM_SUFFIX}.json"))
