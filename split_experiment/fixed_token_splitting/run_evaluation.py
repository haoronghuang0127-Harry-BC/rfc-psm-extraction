...
from config.paths import FIXED_TOKEN_SPLITTING_EVALUATIONS_DIR, FIXED_TOKEN_SPLITTING_FINAL_FSMS_DIR
...
STATE_OUTPUT_FILE: Final[Path] = FIXED_TOKEN_SPLITTING_EVALUATIONS_DIR / "states_match_results.csv"

# complete transition evaluation csv file.
EXACT_TRANSITION_OUTPUT_FILE: Final[Path] = FIXED_TOKEN_SPLITTING_EVALUATIONS_DIR / "transitions_match_results_whole.csv"

# partial transition evaluation csv file.
PARTIAL_TRANSITION_OUTPUT_FILE: Final[Path] = FIXED_TOKEN_SPLITTING_EVALUATIONS_DIR / "transitions_match_results_partial.csv"


# evaluate all fixed token final FSM files.
def run_evaluation() -> tuple[Path, Path, Path]:
    final_fsm_files: list[Path] = sorted(FIXED_TOKEN_SPLITTING_FINAL_FSMS_DIR.glob(f"*{FINAL_FSM_SUFFIX}.json"))
