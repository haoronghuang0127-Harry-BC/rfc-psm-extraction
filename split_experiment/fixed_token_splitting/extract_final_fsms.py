...
from config.paths import FIXED_TOKEN_SPLITTING_COMBINATION_RESPONSES_DIR, FIXED_TOKEN_SPLITTING_FINAL_FSMS_DIR
...
def _get_final_fsm_file_path(response_file_path: Path) -> Path:
    ...
    final_fsm_file_path: Path = FIXED_TOKEN_SPLITTING_FINAL_FSMS_DIR / final_fsm_file_name

    return final_fsm_file_path


# extract and save all fixed token final FSMs.
def extract_final_fsms() -> list[Path]:
    combination_response_files: list[Path] = sorted(FIXED_TOKEN_SPLITTING_COMBINATION_RESPONSES_DIR.glob("*_combination_response.json"))
