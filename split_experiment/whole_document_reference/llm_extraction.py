...
from config.paths import WHOLE_DOCUMENT_REFERENCE_CONTEXT_EXCLUSIONS_FILE, WHOLE_DOCUMENT_REFERENCE_ORIGINAL_RESPONSES_DIR, WHOLE_DOCUMENT_REFERENCE_PROMPTS_DIR
...
def _load_extraction_prompts(protocol: str) -> dict[str, list[str]]:
    prompts_file: Path = WHOLE_DOCUMENT_REFERENCE_PROMPTS_DIR / "ollama_json_schema_output_extraction_prompts.json"
...
def _build_save_response_path(protocol: str, model_name: str, profile_name: ProfileName) -> Path:
    ...
    output_file: Path = WHOLE_DOCUMENT_REFERENCE_ORIGINAL_RESPONSES_DIR / file_name

    return output_file


# count Prompt tokens with the fixed experiment tokenizer.
def _count_prompt_tokens(prompt: str) -> int:
    token_encoding = tiktoken.get_encoding("cl100k_base")

    prompt_token_count: int = len(token_encoding.encode(prompt))

    return prompt_token_count


# build one context exclusion record.
def _build_context_exclusion(protocol: str, model_config: ModelConfig, profile_name: ProfileName, model_profile: ModelProfile, prompt_token_count: int) -> WholeDocumentContextExclusion:
    ...
    context_exclusion: WholeDocumentContextExclusion = {
        "protocol": protocol,
        ...
        "reason": "The complete Prompt is larger than the available input context.",
    }

    return context_exclusion


# run one whole document extraction.
def _run_extraction_psm(protocol: str, prompt: str, model_config: ModelConfig, profile_name: ProfileName, model_profile: ModelProfile, output_control: OutputControl, connection: OllamaConnection) -> Path:
    print(f"Starting extraction: protocol={protocol}, model={model_config['name'].value}, profile={profile_name.value}, output=ollama_json_schema_output, prompts=1")
    ...
    response_copy["prompt_index"] = 1

    response_records: list[dict[str, object]] = [response_copy]
...
# run whole document extraction for the selected protocols and models.
def extraction_psm(extraction_prompts: dict[str, list[str]], arguments: SplitExperimentArguments) -> list[Path]:
    ...
    context_exclusions: list[WholeDocumentContextExclusion] = []
    ...
    print(f"Saved {len(context_exclusions)} context exclusions: {WHOLE_DOCUMENT_REFERENCE_CONTEXT_EXCLUSIONS_FILE}")
